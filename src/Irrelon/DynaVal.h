...
struct DynaVal {
	...
	typename = std::enable_if_t<
		...
		>
	>
	DynaVal (T n) : type(DynaValType::Float), number(static_cast<float>(n)) {}

	// Float
	DynaVal (const float num) : type(DynaValType::Float), number(num) {}

	// Int
	DynaVal (const int num) : type(DynaValType::Int), integer(num) {}

	// Uint
	DynaVal (const uint8_t num) : type(DynaValType::Uint), u_integer(num) {}
	DynaVal (const uint16_t num) : type(DynaValType::Uint), u_integer(num) {}
	DynaVal (const uint32_t num) : type(DynaValType::Uint), u_integer(num) {}
	DynaVal (const uint64_t num) : type(DynaValType::Uint), u_integer(num) {}
	...
	[[nodiscard]] float toFloat (const bool looseType = false) const {
		if (looseType) {
			...
			if (type == DynaValType::Uint) {
				return static_cast<float>(u_integer);
			}
	...
	[[nodiscard]] int toInt (const bool looseType = false) const {
		if (looseType) {
			...
			if (type == DynaValType::Uint) {
				return static_cast<int>(u_integer);
			}
	...
	[[nodiscard]] uint toUInt (const bool looseType = false) const {
		if (looseType) {
			...
			if (type == DynaValType::Int) {
				return static_cast<u_int>(integer);
			}
	...
	[[nodiscard]] std::string toString (const bool interpretArrayData = false) const {
		switch (type) {
			...
			case DynaValType::Uint:
				return std::to_string(u_integer);
	...
	DynaVal &set (uint8_t val) {
		...
		u_integer = static_cast<uint>(val);
		return *this;
	}
	...
	bool operator== (const int other) const {
		if (type == DynaValType::Int) return integer == other;
		if (type == DynaValType::Uint) return u_integer == static_cast<u_int>(other);
		if (type == DynaValType::Float) return number == static_cast<float>(other);
		return false;
	}

	bool operator== (const u_int other) const {
		if (type == DynaValType::Uint) return u_integer == other;
		if (type == DynaValType::Int) return integer == static_cast<int>(other);
		if (type == DynaValType::Float) return number == static_cast<float>(other);
		return false;
	}

	bool operator== (const float other) const {
		if (type == DynaValType::Float) return number == other;
		if (type == DynaValType::Int) return static_cast<float>(integer) == other;
		return false;
	}
	...
	bool operator== (const DynaVal &other) const {
		switch (type) {
			...
			case DynaValType::Int:
				if (other.type == DynaValType::Int) return integer == other.integer;
				if (other.type == DynaValType::Uint) return integer == static_cast<int>(other.u_integer);
			case DynaValType::Uint:
				if (other.type == DynaValType::Uint) return u_integer == other.u_integer;
				if (other.type == DynaValType::Int) return u_integer == static_cast<u_int>(other.integer);
			case DynaValType::Float:
	...
	[[nodiscard]] DynaVal deepCopy () const {
		switch (type) {
			...
			case DynaValType::Int:
				return DynaVal(integer);
			case DynaValType::Uint:
				return DynaVal(u_integer);
			case DynaValType::Float:
				return DynaVal(number);
			case DynaValType::Bool:
